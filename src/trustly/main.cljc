(ns trustly.main
  "Kotodama WASM entrypoint for the Trustly clean-room actor (L4) — Clojure port.
  ...
  facts (`trustly.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "trustly")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"trustly.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "trustly-compat" :tier tier :entities entities} 200])
...
(defn start! [] :trustly-compat/ready)
